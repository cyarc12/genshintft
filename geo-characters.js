...
(() => {
  ...
  SKILL_INFO.凝光 =
    '初始法力10/60。星璇爆发：向当前目标发射一枚大型岩元素法球；法球接触目标后，造成250/400/700 + 攻击力×220%/280%/380%的岩元素伤害并施加岩元素附着。单体技能，不造成范围伤害；目标失效时重新选择合法目标。';
  SKILL_BRIEF.凝光 =
    '大型岩元素法球命中后，造成250/400/700 + 攻击力×220%/280%/380%的单体岩伤并附着。';
  SKILL_INFO.纳维亚 =
  ...
  function resolveHit(projectile, source, target) {
    ...
    if (projectile.kind === 'ningguang') {
      raw = valueForStar(source, [250, 400, 700]) +
        effectiveAtk(source) * valueForStar(source, [2.2, 2.8, 3.8]);
    } else {
