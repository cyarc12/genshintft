...
function renderSlotList(){const slots=loadSlots(),box=$('#pveSlotList');box.innerHTML=SLOT_DEFS.map(s=>{const enemies=slots[s.key]||[],label=s.label.replace('·','·<wbr>'),preview=enemies.map(saved=>{const entry=Object.entries(PIECE_CONFIG).find(([,config])=>config.templateId===saved.templateId),name=entry?.[0]||'未知棋子',config=entry?.[1],color=ELEMENTS[config?.element]||'#70839d';return`<span class="pve-slot-unit" style="--slot-element:${color}" title="${name}"><img src="${config?.avatar||''}" alt="${name}"></span>`}).join('');return`<button data-slot-key="${s.key}" class="${selectedSlot===s.key?'active':''}"><span class="pve-slot-name">${label}</span>${preview?`<span class="pve-slot-units">${preview}</span>`:'<small class="pve-slot-empty">未保存</small>'}</button>`}).join('')}
...
function installShopLayoutStyle(){
  ...
  style.textContent=`
    ...
    #pveSlotDialog .pve-slot-list button{grid-template-columns:88px minmax(0,1fr);grid-template-rows:62px;min-height:86px;padding:10px 6px;font-size:14px;column-gap:3px}
    #pveSlotDialog .pve-slot-name{align-self:start;width:88px;line-height:25px;white-space:normal;word-break:keep-all}
