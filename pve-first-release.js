...
function openChallengeSettlement(){if(mode!=='challenge'||!run)return;$('#challengeSettlementStage').textContent=`当前关卡：${stageId()||'已完成'}`;$('#challengeSettlementRound').textContent=`已完成回合：${run.round}`;$('#challengeSettlementDialog').classList.remove('hidden')}
...
function installShopLayoutStyle(){
  ...
  style.textContent=`
    ...
    .pve-challenge-settlement{width:min(480px,calc(100% - 30px));text-align:center}
