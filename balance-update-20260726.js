...
function createVaporMark(source,target,element,triggerDamage=0,reactionContext=null){
  ...
  if(mark){
    ...
    mark.triggerCount=2;
    mark.rate=(Number(mark.rate)||.60)+.90;
...
function settleConfirmedVaporMark(mark){
  ...
  const rate=Math.max(0,Number(mark.rate)||.60);
...
tick=function(dt){
  ...
  for(const unit of units){
    if(!unit.zhongliShield)continue;
    unit.zhongliShield.time-=dt;
    if(unit.zhongliShield.time<=0)clearZhongliShield(unit,'自然到期');
  }
  for(const wave of confirmedNeuvilletteFloorWaves){
    wave.time+=dt;
    for(const cell of wave.cells){
...
tickAmberBombs=function(dt){
  for(const b of amberBombs){
    b.time+=dt;if(b.resolved||b.time<b.explodeAt)continue;b.resolved=true;
    const flat=confirmedValue(b.source,[150,250,2000]);
    const ratio=confirmedValue(b.source,[1,2,5]);
    for(const e of confirmedEnemies(b.source).filter(x=>dist(x,b.cell)<=1.5))
      confirmedElementHit(b.source,e,flat+effectiveAtk(b.source)*ratio,true);
  }
...
Object.assign(SKILL_INFO,{
  ...
  '安柏':'初始法力40/120。\\n【R·爆弹玩偶】优先在能够覆盖当前攻击目标的位置中选择敌人最密集的落点；没有合法落点时选择全棋盘最密集位置。延迟1秒后对半径1.5格敌人造成150/250/2000 + 攻击力100%/200%/500%的火元素伤害，全部附着。',
...
const confirmedReactionGuide=[
  ['蒸发','火 + 水','生成4秒蒸发标记，记录目标实际损失的护盾与生命；首次触发结算系数为60%，第二次实际触发蒸发后提高至150%且不延长时间。标记结算前第三次及后续触发无效。'],
